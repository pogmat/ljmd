...
int read_brodc_input_params(int proc_id, mdsys_t *sys, FILE *infile, char* restfile, file_names *fnames, int *nprint) {
...
int mpi_initialise(int nprocs, int proc_id, arr_seg_t *proc_seg, mdsys_t *sys, FILE *infile, file_names *fnames, int *nprint) {
	...
	int init_out = read_brodc_input_params( proc_id, sys, infile, restfile, fnames, nprint );
	if (init_out ) {
	...
	*/
	...
	/* 	velocity memory is  handled differently than positions or forces memory
		master needs a full-size buffer for input but for simulation
		every process will just need a buffer sized according to number of particles
	*/
	...
	/* 	initialise arrays for MPI Scatterv
		...
		offset indicates the beginning of the segment in the master buffer
	*/
	...
	for ( int p=0; p < nprocs; ++p) { 
		if (p < proc_seg->splitting[0] ) {
			...
			offsets[p] = proc_seg->splitting[0]*proc_seg->splitting[1] + (p - proc_seg->splitting[0])*proc_seg->splitting[2];
		}
	}
	...
	MPI_Iscatterv( vxbuf, count, offsets, MPI_DOUBLE, sys->vx, proc_seg->size,  MPI_DOUBLE, 0,MPI_COMM_WORLD, &vxreq); 
	MPI_Iscatterv( vybuf, count, offsets, MPI_DOUBLE, sys->vy, proc_seg->size,  MPI_DOUBLE, 0,MPI_COMM_WORLD, &vyreq); 
	MPI_Iscatterv( vzbuf, count, offsets, MPI_DOUBLE, sys->vz, proc_seg->size,  MPI_DOUBLE, 0,MPI_COMM_WORLD, &vzreq); 
